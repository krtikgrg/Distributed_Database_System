...
class Query:
    ...
    def generateTree(self):
        ...
        for relation in self.relations:
            ...
            columns = copy.deepcopy(config.relationColumnMap[relation])
        ...
        if self.HAVE_JOIN:
            ...
            for i in range(len(self.join_conditions['relation1'])):
                ...
                currentNode.children.append(nodeConsider[currentNode.r1])
                currentNode.children.append(nodeConsider[currentNode.r2])
                currentNode.generate_attributes_list()
                nodeConsider[currentNode.r1].parent = currentNode
                nodeConsider[currentNode.r1] = currentNode
                nodeConsider[currentNode.r2].parent = currentNode
                nodeConsider[currentNode.r2] = currentNode

            #Validation Check
            defect = 0
            for i in nodeConsider:
                if finalNode is None:
                    finalNode = nodeConsider[i]
                else:
                    if nodeConsider[i] != finalNode:
                        defect = 1
                        break
            if defect:
                config.debugPrint("Error in joins, joins do not converge to one in the end")
            else:
